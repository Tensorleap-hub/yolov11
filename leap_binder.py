...
def metadata_metric(y_pred: np.ndarray, preprocess: SamplePreprocessResponse):
    ...
    d = {
        "gt empty pred invent (FP)": float(1.) if len(clss_info[0])!=0 and len(np.unique(cls_gt, return_counts=True)[0])==0 else 0.0,
        "pred empty gt not (FN)": float(1.) if len(clss_info[0]) == 0 and len(
            np.unique(cls_gt, return_counts=True)[0]) != 0 else 0.0,
    ...
    d["count of 'giraffe' class (23)"]=count_dict["count of 'giraffe' class (23)"]
    d["count of 'airplane' class (4)"]=count_dict["count of 'airplane' class (4)"]
